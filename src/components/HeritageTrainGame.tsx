  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-start bg-sky-50 p-4 relative overflow-hidden">
      ...
      <div className="bg-white/90 backdrop-blur-sm p-4 rounded-2xl shadow-md border-2 border-white text-center mb-4 z-10">
        <h2 className="text-3xl font-black text-stone-600">
          Starts with: <span className="text-5xl text-red-500">{targetLetter}</span>
        </h2>
      </div>
          ...
          <AnimatePresence>
            {options.map((opt) => (
              ...
              >
                ...
                <span className="text-xl font-bold text-stone-700 uppercase">{opt.english}</span>
              </motion.button>
            ))}
          </AnimatePresence>
        </div>
