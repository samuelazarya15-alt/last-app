...
export const CoffeeCeremonyGame: React.FC<CoffeeCeremonyGameProps> = ({
  ...
  const startGame = () => {
    ...
    nextRound(1);
    setDoveMessage("Find the right item for the ceremony!");
  };

  const nextRound = useCallback((currentLevel: number) => {
    const foodWords = words.filter(w => w.category === 'Food' || w.category === 'Household');
    ...
    const target = foodWords[Math.floor(Math.random() * foodWords.length)];
    setCurrentTarget(target);
    
    const opts = new Set<any>();
    opts.add(target);
    while (opts.size < Math.min(numOptions, foodWords.length)) {
  ...
  useEffect(() => {
    if (gameState === 'playing' && timeLeft > 0) {
      ...
      setGameState('end');
      logGameSession('coffee', score, 60);
            ...
            <div className="grid grid-cols-2 gap-6 w-full overflow-y-auto max-h-[50vh] p-2">
              {options.map((item) => (
                <motion.button
                  ...
                  className="bg-white p-8 rounded-[2rem] shadow-lg border-4 border-transparent hover:border-stone-400 transition-all flex flex-col items-center justify-center gap-2"
                >
                  <span className="text-5xl">{item.emoji}</span>
                  <span className="text-2xl font-geez font-black text-stone-800">{item.translations.tigrinya}</span>
                </motion.button>
              ))}
            </div>
          </motion.div>
